...
export class auth {
  ...
  async loginUser(data: LoginUserDto): Promise<any> {
    ...
    if (!user) {
      throw new NotFoundException("Account with the email doesn't exists. Please register");
    }
   ...
   * @param userId
   * @param email
   * @returns REFRESH AND ACCESS TOKEN SIGNED FOR THE USER
   * @description This function signs a token for the user
   */
